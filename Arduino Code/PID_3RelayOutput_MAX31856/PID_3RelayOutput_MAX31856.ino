...
void loop()
{
  ...
  if (iState == STATE_START_MSG)
  {
    ...
    LCD_PrintLine1("HeatPad PID Ctrl");
    LCD_PrintLine2("Btn Push START");
  ...
  if (iState == STATE_SETTING_MSG)
  {
    lcd.clear();
    LCD_PrintLine1("HeatPad PID Ctrl");
    LCD_PrintLine2("Btn Push SETTING");
  ...
  {
    lcd.clear();
    LCD_PrintLine1("Select Setting  ");
    iSettingSelect = SELECT_SETTING_1;
    LCD_PrintLine2(sSettingSelect[iSettingSelect-1]);
  ...
  {
    lcd.clear();
    LCD_PrintLine1(sHeatingPad[iSelectHeadingPad-1]);
...
//        writeSetpointToEEPROM(EEPROM_SETPOINT_1+((iSelectHeadingPad-1)*3), Setpoint[iSelectHeadingPad-1]);
  ...
  {
    lcd.clear();
    LCD_PrintLine1("HeatPad Settings");
    iSelectHeadingPad = SELECT_HEATING_PAD_1;
    LCD_PrintLine2(sHeatingPad[iSelectHeadingPad-1]);
...
/*
      lcd.setCursor(3, 0);
      lcd.print(int(SetpointNew)/10);
      lcd.print(int(SetpointNew)%10);
      lcd.print(".");
      lcd.print(int(SetpointNew*10)%10);
      lcd.print(" [");
      lcd.setCursor(9, 0);
      lcd.print(int(Setpoint[iSelectHeadingPad-1])/10);
      lcd.print(int(Setpoint[iSelectHeadingPad-1])%10);
      lcd.print(".");
      lcd.print(int(Setpoint[iSelectHeadingPad-1]*10)%10);
      lcd.print("]");
*/
      LCD_PrintLine2("   OK    ");
...
void showValue(int iRow, int iCol, float fValue)
{
  ...
  char *sLine = sLCD_Line2;
  if (iRow == 0)
    sLine = sLCD_Line1;
  //sLine = &sLine[iCol];
//  for (int i=0; i<iCol; i++)
//    sLine[i] = ' ';
  sLine[iCol+0] = '0' + int(fValue)/10;
  sLine[iCol+1] = '0' + int(fValue)%10;
  sLine[iCol+2] = '.';
  sLine[iCol+3] = '0' + int(fValue*10)%10;
  for (int i=iCol+4; i<16; i++)
    sLine[i] = ' ';
  if (iRow == 0)
    LCD_PrintLine1(sLine);
  else
    LCD_PrintLine2(sLine);
}
...
void LCD_PrintLine1(char *sLine)
{
  strcpy(sLCD_Line1, sLine);
  lcd.setCursor(0,0);
  lcd.print(sLCD_Line1);
}

void LCD_PrintLine2(char *sLine)
{
  strcpy(sLCD_Line2, sLine);
  lcd.setCursor(0,1);
  lcd.print(sLCD_Line2);
}
